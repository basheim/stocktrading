...
def buy(assessment: HistoryAssessment, account: float):
    logger.info(f"Buy transaction for {assessment.code} started...")
    try:
        ...
        logger.error("Unable to execute buy action")
        return
    ...
    logger.info(f"Buy transaction for {assessment.code} completed with quantity {assessment.owned} and price {assessment.owned_price}")


def sell(assessment: HistoryAssessment):
    logger.info(f"Sell transaction for {assessment.code} started...")
    try:
        order = execute_sell(assessment.code, assessment.owned)
    except (APIError, ValidationError) as e:
        logger.error("Unable to execute sell action")
        return
